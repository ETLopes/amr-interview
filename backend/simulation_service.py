...
class SimulationService:
    ...
    @staticmethod
    def calculate_simulation_values(property_value: float, down_payment_percentage: float, contract_years: int):
        """
    ...
    def get_user_simulations(db: Session, user_id: int, skip: int = 0, limit: int = 100):
        ...
        total = db.query(models.Simulation).filter(
            models.Simulation.user_id == user_id
        ).count()
    ...
    @staticmethod
    def update_simulation(db: Session, simulation_id: int, simulation_data: schemas.SimulationUpdate, user_id: int):
        """
        ...
        if any(key in update_data for key in ['property_value', 'down_payment_percentage', 'contract_years']):
            # Get current values or use updated ones
            property_value = update_data.get('property_value', db_simulation.property_value)
            down_payment_percentage = update_data.get('down_payment_percentage', db_simulation.down_payment_percentage)
